...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/05/08 19:11:02 by abodnar           #+#    #+#             */
/*   Updated: 2018/05/08 19:11:02 by abodnar          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_asm_abodnar.h"

t_handle_func	ft_asm_regex_aox_reg_third(char **str, int *res,
												t_asm_valid *params)
{
	size_t	len;
	char	*str_cpy;
												...
												t_asm_valid *params)
{
	...
	if (**str == SEPARATOR_CHAR)
	{
		...
		while (**str && (**str == ' ' || **str == '\t'))
			ft_asm_skip_symb(str, res);
		if (**str == REG_CHAR)
			return ((void*)ft_asm_regex_aox_reg_third);
	}
	return (NULL);
}
														...
														t_asm_valid *params)
{
	...
	if (label_len > 0)
	{
		...
		return ((void*)ft_asm_regex_aox_third_arg);
	}
	return (NULL);
}
...
t_handle_func	ft_asm_regex_aox_dir_indir_second(char **str, int *res,
													t_asm_valid *params)
{
	size_t	nb_len;
	...
	if (**str == '-')
		...
		return ((void*)ft_asm_regex_aox_save_label_second);
	nb_len = 0;
...
t_handle_func	ft_asm_regex_aox_second_arg(char **str, int *res,
												t_asm_valid *params)
{
	(void)params;
	while (**str && (**str == ' ' || **str == '\t'))
	...
	if (**str == SEPARATOR_CHAR)
	{
		...
		while (**str && (**str == ' ' || **str == '\t'))
			ft_asm_skip_symb(str, res);
		if (**str == REG_CHAR)
			return ((void*)ft_asm_regex_aox_reg_second);
		else if (**str == DIRECT_CHAR)
		{
			ft_asm_skip_symb(str, res);
			return ((void*)ft_asm_regex_aox_dir_indir_second);
		}
...
t_handle_func	ft_asm_regex_aox_save_label_first(char **str, int *res,
													t_asm_valid *params)
{
	size_t	label_len;
	t_label	*new_label;
	...
	if (label_len > 0)
	{
		...
		new_label->label = ft_strsub(*str - label_len, 0, label_len);
		ft_al_insert(params->labels_args, new_label);
		return ((void*)ft_asm_regex_aox_second_arg);
	}
	return (NULL);
}
													...
													t_asm_valid *params)
{
	...
	if (**str == '-')
		...
		return ((void*)ft_asm_regex_aox_save_label_first);
	nb_len = 0;
	while (**str && **str != ' ' && **str != '\t' && **str != SEPARATOR_CHAR)
	{
		if (ft_isdigit(**str) && ft_asm_skip_symb(str, res))
												...
												t_asm_valid *params)
{
	...
	while (**str && (**str == ' ' || **str == '\t'))
		ft_asm_skip_symb(str, res);
	if (**str == REG_CHAR)
		return ((void*)ft_asm_regex_aox_reg_first);
	else if (**str == DIRECT_CHAR)
	{
		ft_asm_skip_symb(str, res);
		return ((void*)ft_asm_regex_aox_dir_indir_first);
	}
	else if (**str == '-' || **str == LABEL_CHAR || ft_isdigit(**str))
		return ((void*)ft_asm_regex_aox_dir_indir_first);
	return (NULL);
...
t_handle_func	ft_asm_regex_and_after_label(char **str, int *res,
												t_asm_valid *params)
{
	int		i;
	int		cmd_name_len;
	...
	while (**str && (**str == ' ' || **str == '\t'))
		ft_asm_skip_symb(str, res);
	cmd_name_len = (int)ft_strlen(g_op_tab[5].name);
	if (ft_strnequ(*str, g_op_tab[5].name, (size_t)cmd_name_len))
	{
		i = -1;
		while (++i < cmd_name_len)
			ft_asm_skip_symb(str, res);
		return ((void*)ft_asm_regex_aox_first_arg);
	}
	return (NULL);
}

t_handle_func	ft_asm_regex_and_save_start_label(char **str, int *res,
														t_asm_valid *params)
{
	int		label_len;
	char	*new_label;
	...
	if (ft_asm_regex_is_duplicated_label(new_label, params))
	{
		ft_strdel(&new_label);
		ft_asm_skip_symb(str, res);
		return ((void*)ft_asm_regex_and_after_label);
	}
...
t_handle_func	ft_asm_regex_and_start(char **str, int *res,
											t_asm_valid *params)
{
	char	*label_end;
	...
	if ((label_end = ft_strchr(*str, LABEL_CHAR)) != NULL
		...
		return ((void*)ft_asm_regex_and_save_start_label);
	return ((void*)ft_asm_regex_and_after_label);
}
...
int				ft_asm_regex_and(char *champ, t_asm_valid *params)
{
	...
	(void)params;
	func_curr = (t_handle_func)ft_asm_regex_and_start;
	res = 1;
