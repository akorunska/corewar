...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/05/08 19:05:34 by abodnar           #+#    #+#             */
/*   Updated: 2018/05/08 19:05:34 by abodnar          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
													...
													t_asm_valid *params)
{
	...
	{
		ft_asm_skip_symb(str, res);
		return ((void*)ft_asm_regex_ld_dir_indir_first);
	}
...
t_handle_func	ft_asm_regex_ld_after_label(char **str, int *res,
													t_asm_valid *params)
{
	int		i;
	int		cmd_name_len;
	...
	while (**str && (**str == ' ' || **str == '\t'))
		ft_asm_skip_symb(str, res);
	cmd_name_len = (int)ft_strlen(g_op_tab[1].name);
	if (ft_strnequ(*str, g_op_tab[1].name, (size_t)cmd_name_len))
	{
		i = -1;
		while (++i < cmd_name_len)
			ft_asm_skip_symb(str, res);
		return ((void*)ft_asm_regex_ld_lld_first_arg);
	}
	return (NULL);
}

t_handle_func	ft_asm_regex_ld_save_start_label(char **str, int *res,
														t_asm_valid *params)
{
	int		label_len;
	char	*new_label;
	...
	if (ft_asm_regex_is_duplicated_label(new_label, params))
	{
		ft_strdel(&new_label);
		ft_asm_skip_symb(str, res);
		return ((void*)ft_asm_regex_ld_after_label);
	}
...
t_handle_func	ft_asm_regex_ld_start(char **str, int *res,
											t_asm_valid *params)
{
	char	*label_end;
	...
	if ((label_end = ft_strchr(*str, LABEL_CHAR)) != NULL
		...
		return ((void*)ft_asm_regex_ld_save_start_label);
	return ((void*)ft_asm_regex_ld_after_label);
}
...
int				ft_asm_regex_ld(char *champ, t_asm_valid *params)
{
	...
	(void)params;
	func_curr = (t_handle_func)ft_asm_regex_ld_start;
	res = 1;
