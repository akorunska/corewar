...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ft_asm_regex_lfork.c                               :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: abodnar <marvin@42.fr>                     +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/05/14 15:53:10 by abodnar           #+#    #+#             */
/*   Updated: 2018/05/14 15:53:12 by abodnar          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_asm_abodnar.h"

t_handle_func	ft_asm_regex_lfork_after_label(char **str, int *res,
													t_asm_valid *params)
{
	int		i;
	int		cmd_name_len;
	...
	cmd_name_len = (int)ft_strlen(g_op_tab[14].name);
	if (ft_strnequ(*str, g_op_tab[14].name, (size_t)cmd_name_len))
	{
		i = -1;
		while (++i < cmd_name_len)
			ft_asm_skip_symb(str, res);
		return ((void*)ft_asm_regex_live_zjmp_fork_lfork_first_arg);
	}
	return (NULL);
}

t_handle_func	ft_asm_regex_lfork_save_start_label(char **str, int *res,
														t_asm_valid *params)
{
	int		label_len;
	char	*new_label;
	...
	if (ft_asm_regex_is_duplicated_label(new_label, params))
	{
		ft_strdel(&new_label);
		ft_asm_skip_symb(str, res);
		return ((void*)ft_asm_regex_lfork_after_label);
	}
...
t_handle_func	ft_asm_regex_lfork_start(char **str, int *res,
												t_asm_valid *params)
{
	char	*label_end;
	...
	{
		if (ft_strchr(*str, COMMENT_CHAR) != NULL
			&& label_end < ft_strchr(*str, COMMENT_CHAR))
			return ((void*)ft_asm_regex_lfork_save_start_label);
		else if (ft_strchr(*str, COMMENT_CHAR_ADD) != NULL
				 && label_end < ft_strchr(*str, COMMENT_CHAR_ADD))
			return ((void*)ft_asm_regex_lfork_save_start_label);
		return ((void*)ft_asm_regex_lfork_save_start_label);
	}
	return ((void*)ft_asm_regex_lfork_after_label);
...
int				ft_asm_regex_lfork(char *champ, t_asm_valid *params)
{
	...
	(void)params;
	func_curr = (t_handle_func)ft_asm_regex_lfork_start;
	res = 1;
