...
t_handle_func	ft_asm_regex_st_save_label_second(char **str, int *res,
														t_asm_valid *params)
{
	size_t	label_len;
	t_label	*new_label;
...
t_handle_func	ft_asm_regex_st_indir_second(char **str, int *res,
													t_asm_valid *params)
{
	size_t	nb_len;
	...
	if (**str == '-')
		...
		return ((void*)ft_asm_regex_st_save_label_second);
	nb_len = 0;
	while (**str && **str != ' ' && **str != '\t' && **str != '\n'
			&& **str != COMMENT_CHAR_ADD && **str != COMMENT_CHAR_ADD)
	{
...
t_handle_func	ft_asm_regex_st_reg_second(char **str, int *res,
													t_asm_valid *params)
{
	size_t	len;
	char	*str_cpy;
													...
													t_asm_valid *params)
{
	...
	if (**str == SEPARATOR_CHAR)
	{
		...
		while (**str && (**str == ' ' || **str == '\t'))
			ft_asm_skip_symb(str, res);
		if (**str == REG_CHAR)
			return ((void*)ft_asm_regex_st_reg_second);
		else if (**str == '-' || ft_isdigit(**str) || **str == LABEL_CHAR)
			return ((void*)ft_asm_regex_st_indir_second);
	}
	return (NULL);
}
